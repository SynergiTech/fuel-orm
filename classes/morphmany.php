...
class MorphMany extends Relation
{
	public function __construct($from, $name, array $config)
	{
		...
		$this->model_type    = array_key_exists('model_type', $config)
			? $config['model_type'] : null;
		...
		{
			throw new \FuelException('Related model not found by MorphMany relation "'.$this->name.'": '.$this->model_to);
		}
	...
	public function join($alias_from, $rel_name, $alias_to_nr, $conditions = array())
	{
		...
		foreach ($this->key_from as $key)
		{
			...
			$model['join_on'][] = array($alias_to.'.'.$this->model_type, '=', \DB::escape($this->model_from));
			next($this->key_to);
		}
		...
		foreach ($models_to as $key => $model_to)
		{
			...
			if (($model_to and $model_to->is_new()) or ! in_array($current_model_id, $original_model_ids))
			{
				...
				foreach ($this->key_from as $pk)
				{
					...
					$model_to->{$this->model_type} = $this->model_from;
					next($this->key_to);
				}
